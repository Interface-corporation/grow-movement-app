...
import { Loader2, Eye, X, User, Briefcase, Hash, XCircle, Clock, CheckCircle, AlertTriangle } from 'lucide-react';
...
export default function CoachMatchingRequests() {
  ...
  const cancelRequest = async (id: string) => {
    setCancelling(id);
    await supabase.from('matching_requests').update({ status: 'cancelled' }).eq('id', id);
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-sm text-muted-foreground">View the status of your matching requests and cancel pending ones.</p>
        ...
        <div className="space-y-3">
          {requests.map(req => (
            <div key={req.id} className="bg-card rounded-2xl border border-border p-4 hover:shadow-sm transition-shadow">
              ...
              {/* Show selected entrepreneurs preview */}
